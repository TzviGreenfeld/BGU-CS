...
public abstract class MicroService implements Runnable {
  ...
  protected HashMap<Class, Callback> eventToCallback;
   ...
   *                 queue.
   */
  protected final <T, E extends Event<T>> void subscribeEvent(
    Class<E> type,
    Callback<E> callback
  ) {
   ...
   *                 queue.
   */
  protected final <B extends Broadcast> void subscribeBroadcast(
    Class<B> type,
    Callback<B> callback
  ) {
  ...
  protected final void terminate() {
    ...
    // messageBus.sendBroadcast(new CloseAllBroadcast());
  }
  ...
  public final void run() {
    ...
    while (!done) {
      try {
        // execute the callback of the mag we got
        Message m = messageBus.awaitMessage(this);
        eventToCallback.get(m.getClass()).call(m);
      } catch (InterruptedException ignored) {}
    }
    System.out.println("finish " + msName);
