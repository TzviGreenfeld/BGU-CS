...
public class GPUService extends MicroService {
    ...
    protected void initialize() {
            ...
            if (busy) {
                ...
                if (gpu.isCurentlyTraining()) {
                    if (gpu.getFinishTime() == gpu.getTime()) {
                        ...
                        } else { // vram is empty when were done training the model
                            model.setStatus(Model.Status.Trained);
                            busy = false;
                            gpu.setCurentlyTraining(false);
                            complete(currentModelEvent, model);
                            gpu.addTrainedModel(model.getName());
                            /// if thers an modelEvent waiting, handle i now
        ...
        this.subscribeEvent(TrainModelEvent.class, c -> {
            trainModel(c);

        });

        this.subscribeEvent(TestModelEvent.class, c -> {
            Boolean isGood = null;
