...
int move_to_swapfile(struct proc *p, int pageIndexInSwapFile)
{
  ...
  p->swappedToDisk[pageIndexInSwapFile].swapFileOffset = pageIndexInSwapFile * PGSIZE;
...
int move_to_ram(uint64 v_addr, pte_t *pte, char *phys_addr, struct proc *proc, int ramPageIdx)
{
  ...
  // Update metadata of the corresponding entry in the swappedToDisk array
  proc->swappedToDisk[swapPageIdx].virtualAddr = UNUSED;
  proc->swappedToDisk[swapPageIdx].state = UNUSED;

  proc->memory[ramPageIdx].virtualAddr = v_addr;
...
int get_first_ram_unused(struct proc *p)
{
  ...
  while (pageIndex < MAX_PSYC_PAGES)
  {
    if (p->memory[pageIndex].state == UNUSED)
    {
      return pageIndex;
    }
    ++pageIndex;
  }
  return -1; // Return -1 if no unused page is found
}

int get_first_swapped_unused(struct proc *p)
{
  int pageIndex = 0;

  while (pageIndex < MAX_PSYC_PAGES)
  {
    if (p->swappedToDisk[pageIndex].state == UNUSED)
    {
      return pageIndex;
    }
    ++pageIndex;
...
int mappages_single_page(pagetable_t pagetable, uint64 va, uint64 size, uint64 pa, int perm)
{
  uint64 a, last;
  pte_t *pte;
...
int findVirtualAddrIndex(struct proc *p, uint a)
{ // ToDo: Check in Tzvi code if this function is needed.
  for (int i = 0; i < MAX_PSYC_PAGES; i++)
  {
    if (p->memory[i].virtualAddr == a)
    {
      return i;
    }
  }
  return -1; // return -1 if the virtual address was not found
}
...
#ifndef NONE
    struct proc *p = myproc();
    if (not_shell_or_init(p->pid))
    {
      int index = findVirtualAddrIndex(p, a);
      if (index != -1)
      {
        initialize_process_arrays(p, index);
      }
    }
#endif
  }
...
// Load the user initcode into virtualAddr 0 of pagetable,
...
void setMemoryState(struct proc *process, int ramPageIndex, int swapFilePageIndex, uint64 addr)
{
  process->memory[ramPageIndex].state = USED;
  process->memory[ramPageIndex].virtualAddr = addr;
  process->memory[ramPageIndex].initDate = generate_unique_time();

#if LAPA
  process->memory[ramPageIndex].accsesses = 0xFFFFFFFF;
#endif

#if NFUA
  process->memory[ramPageIndex].accsesses = 0;
#endif

  process->memory[ramPageIndex].swapFileOffset = ramPageIndex * PGSIZE;
}
...
uvmalloc(pagetable_t pagetable, uint64 oldsz, uint64 newsz, int xperm)
{
  ...
  for (addr = oldsz; addr < newsz; addr += PGSIZE)
  {
    ...
    memset(mem, 0, PGSIZE);
    if (mappages(pagetable, addr, PGSIZE, (uint64)mem, PTE_R | PTE_U | xperm) != 0)
    {
      kfree(mem);
      uvmdealloc(pagetable, addr, oldsz);
      return 0;
    ...
    if (not_shell_or_init(p->pid) && p->pagetable == pagetable)
    {
      ...
      if (pageIndexInRam == -1)
      {
        ...
        {
          panic("exceeded memory in uvmalloc");
        }
      }
      setMemoryState(p, pageIndexInRam, pageIndexInSwapFile, addr);
    }
#endif
  }
  return newsz;
  ...
  for (i = 0; i < sz; i += PGSIZE)
  {
    ...
    if ((*pte & PTE_V) == 0)
    {
      ...
      if (*pte & PTE_PG)
      {
        ...
        *new_PTE = (*new_PTE & ~PTE_V) | PTE_PG | PTE_FLAGS(*pte);
      }
      else
...
// Copy len bytes from src to virtual virtualAddr dstva in a given page table.
...
// Copy len bytes to dst from virtual virtualAddr srcva in a given page table.
// Return 0 on success, -1 on error.
...
// Copy bytes to dst from virtual virtualAddr srcva in a given page table,
// until a '\0', or max.
// Return 0 on success, -1 on error.
